...
class SKE_2019_Sequence_labeling_Processor(DataProcessor):
    ...
    def get_examples(self, data_dir):
        with open(os.path.join(data_dir, "token_in.txt"), encoding='utf-8') as token_in_f:
            with open(os.path.join(data_dir, "token_label_and_one_prdicate_out.txt"),
                      encoding='utf-8') as token_label_out_f:
    ...
    def get_test_examples(self, data_dir):
        with open(os.path.join(data_dir, os.path.join("test", "token_in_and_one_predicate.txt")),
                  encoding='utf-8') as token_in_f:
            token_in_list = [seq.replace("\n", '') for seq in token_in_f.readlines()]
            examples = token_in_list
            return self._create_example(examples, "test")

    def get_token_labels(self):
        BIO_token_labels = ["[Padding]", "[category]", "[##WordPiece]", "[CLS]", "[SEP]", "B-SUB", "I-SUB", "B-OBJ",
                            "I-OBJ", "O"]  # id 0 --> [Paddding]
...
def model_fn_builder(bert_config, num_token_labels, num_predicate_labels, init_checkpoint, learning_rate,
    ...
    def model_fn(features, labels, mode, params):  # pylint: disable=unused-argument
        ...
        if mode == tf.estimator.ModeKeys.TRAIN:
            ...
            def metric_fn(predicate_loss, token_label_per_example_loss, predicate_probabilities, token_label_ids,
                ...
                with open(predicate_output_probabilities_file, "w", encoding='utf-8') as predicate_probabilities_writer:
                    ...
                    for (i, prediction) in enumerate(result):
                        ...
                        predicate_probabilities_line = " ".join(
                            str(sigmoid_logit) for sigmoid_logit in predicate_probabilities) + "\n"
