...
schemas_dict_relation_2_object_subject_type = get_schemas()
...
class Sorted_relation_and_entity_list_Management(File_Management):
    ...
    def __init__(self, TEST_DATA_DIR, MODEL_OUTPUT_DIR, Competition_Mode=False):
        File_Management.__init__(self, TEST_DATA_DIR=TEST_DATA_DIR, MODEL_OUTPUT_DIR=MODEL_OUTPUT_DIR,
                                 Competition_Mode=Competition_Mode)
        # 关系列表 把模型输出的实数值对应为标签
        # self.relationship_label_list = ['丈夫', '上映时间', '专业代码', '主持人', '主演', '主角', '人口数量', '作曲', '作者', '作词', '修业年限',
        #                                 '出品公司', '出版社', '出生地', '出生日期', '创始人', '制片人', '占地面积', '号', '嘉宾', '国籍', '妻子', '字',
        #                                 '官方语言', '导演', '总部地点', '成立日期', '所在城市', '所属专辑', '改编自', '朝代', '歌手', '母亲', '毕业院校',
        #                                 '民族', '气候', '注册资本', '海拔', '父亲', '目', '祖籍', '简称', '编剧', '董事长', '身高', '连载网站',
        #                                 '邮政编码', '面积', '首都']
        schemas = get_schemas()
        self.relationship_label_list = list(schemas.keys())
        self.Competition_Mode = Competition_Mode
    ...
    def model_token_label_2_entity_sort_tuple_list(self, token_in_not_UNK_list, predicate_token_label_list):
        ...
        # 预处理标注数据列表
        predicate_token_label_list = preprocessing_model_token_lable(predicate_token_label_list,
                                                                     len(token_in_not_UNK_list))
        entity_sort_list = []
        entity_part_list = []
...
if __name__ == '__main__':
    ...
    spo_list_manager = Sorted_relation_and_entity_list_Management(TEST_DATA_DIR, MODEL_OUTPUT_DIR,
                                                                  Competition_Mode=Competition_Mode)
